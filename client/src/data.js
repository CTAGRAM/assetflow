// AssetFlow — shared demo data + helpers, ported verbatim from the design
// prototype's project/data.js so every screen renders the exact same sample
// content the mockups were designed against.
//
// IMPORTANT: This is DEMO DATA. The final submission must not rely on it.
// Screens must pull their data through src/api.js; api.js falls back to the
// exports here ONLY where a real backend endpoint does not exist yet, and each
// such fallback is flagged with a loud `[TODO]` so it is a one-line swap later.

const TODAY = '2026-07-12'; // demo "now": Sunday, July 12 2026

// ---------- design tokens (from the prototype) ----------
// Note: the prototype chrome (topbar, cards, forms) actually renders in a
// purple / Plus Jakarta Sans system defined inline in each screen. These `T`
// values are the *status-chip* palette the mockups reference through the
// STATUS / BOOKING_STATUS / MAINT_STAGE maps below. The chrome palette lives
...
export function emp(id) { return byId[id] || null; }
export function empName(id) { return id === 'system' ? 'System' : (byId[id] ? byId[id].name : (deptById[id] ? deptById[id].name + ' (dept)' : id)); }
export function asset(tag) { return byTag[tag] || null; }
export function deptName(id) { return deptById[id] ? deptById[id].name : '—'; }
export function fmtDate(d) { if (!d) return '—'; const dt = new Date(d + (d.length === 10 ? 'T12:00' : '')); return dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: dt.getFullYear() === 2026 ? undefined : 'numeric' }); }
export function fmtDateTime(d) { const dt = new Date(d.replace(' ', 'T')); return dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) + ', ' + dt.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' }); }
export function money(n) { return n ? '$' + n.toLocaleString('en-US') : '—'; }
export function initials(name) { return name.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase(); }
export function daysFromToday(d) { return Math.round((new Date(d + 'T12:00') - new Date(TODAY + 'T12:00')) / 86400000); }
export function isOverdue(d) { return d && daysFromToday(d) < 0; }
...
export function avatarBg(id) { const h = avatarHues[id] || 200; return 'oklch(0.88 0.06 ' + h + ')'; }
export function avatarInk(id) { const h = avatarHues[id] || 200; return 'oklch(0.38 0.09 ' + h + ')'; }

export { TODAY };

// Aggregate export mirroring the prototype's `window.AF`, so screens can import
// one object and reference fields exactly as the mockups did.
export const AF = {
  TODAY, T, STATUS, BOOKING_STATUS, MAINT_STAGE, departments, employees, categories,
  assets, allocations, transfers, bookings, maintenance, audits, notifications, activity,
  personas, role, setRole, me, emp, empName, asset, deptName, fmtDate, fmtDateTime,
  money, initials, daysFromToday, isOverdue, avatarBg, avatarInk,
};
export default AF;
